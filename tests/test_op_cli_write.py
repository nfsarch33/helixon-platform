"""
TDD test: op cli write path on node-a must work.

Background
----------
The op 2.34.1 CLI on node-a hangs indefinitely when `op item create` is
...
2. `op vault list` must return the <vault-name> vault.
...
class OpCliWritePathTest(unittest.TestCase):
    """Verifies the op CLI + 1Password Go SDK bootstrap is functional on node-a."""
    ...
    def test_02_vault_list_contains_cursor_ironclaw(self):
        """op vault list must include the <vault-name> vault."""
    ...
    def test_04_item_create_hangs_without_desktop_app(self):
        """op item create on node-a without the 1Password desktop app hangs.
        ...
        self.assertEqual(
            rc, 124,
            "op item create should still hang on node-a until the 1Password "
    ...
    def test_05_bootstrap_binary_builds(self):
        ...
        repo = Path("/home/user/Code/helixon-platform")
    ...
    def test_06_bootstrap_idempotent(self):
        ...
        if not (repo / "go.mod").exists():
            self.skipTest(f"repo not found at {repo}")
        # Use a dummy password (the bootstrap will skip jason@host-b/host-d which
        ...
        # The jason@host-b/host-d items already exist; bootstrap must report skip.
        ...
        self.assertIn("jason@host-b", combined)
        self.assertIn("jason@host-d", combined)
