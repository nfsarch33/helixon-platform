...
func fixtureRegistry(t *testing.T) string {
	...
	reg := registra.Registry{
		...
		Services: []registra.Service{
			{Name: "alpha", Kind: "core", PrimaryNode: "node-b", Address: "127.0.0.1", Port: 8001, HealthPath: "/healthz", OwnerSprint: "v17714"},
		},
		Nodes: []registra.Node{
			{Alias: "node-b", CanonicalHostname: "node-b.local", TailscaleIP: "100.64.0.2", Role: "central", User: "jason", SSHPort: 22},
		},
		LLMCells: []registra.LLMCell{
			{CellID: "cell-qwen", Node: "node-b", GPUClass: "RTX-4070", ModelID: "qwen3.7", Engine: "vllm", HostPort: 8001, Status: "ready"},
		},
	}
...
func TestRunRegistra_ListFilteredByNode(t *testing.T) {
	dir := fixtureRegistry(t)
	rc, stdout, _ := runCapturing(t, []string{"list", "--node", "node-b", "--registry", filepath.Join(dir, "registry.yaml")})
	if rc != 0 {
		t.Fatalf("list --node: rc = %d", rc)
	}
	if !strings.Contains(stdout, "alpha") {
		t.Errorf("list --node=node-b should include 'alpha': %q", stdout)
	}
}
...
func TestRunRegistra_Nodes(t *testing.T) {
	...
	if !strings.Contains(stdout, "node-b") {
		t.Errorf("nodes body missing 'node-b': %q", stdout)
	}
}
...
func TestRunRegistra_Credential(t *testing.T) {
	...
	reg := registra.Registry{
		...
		CredentialsIndex: []registra.Credential{{
			...
			Vault: "<vault-name>", Category: "API_KEY", OPURI: "op://<vault-name>/My Item/password",
		}},
	}
	if err := writeRegistryYAML(filepath.Join(dir, "registry.yaml"), reg); err != nil {
		t.Fatalf("save: %v", err)
...
func TestRunRegistra_Credential_Unknown(t *testing.T) {
	dir := t.TempDir()
	reg := registra.Registry{RegistryVersion: "v1", SchemaVersion: 1, CentralNode: "node-b"}
	if err := writeRegistryYAML(filepath.Join(dir, "registry.yaml"), reg); err != nil {
		t.Fatalf("save: %v", err)
	}
