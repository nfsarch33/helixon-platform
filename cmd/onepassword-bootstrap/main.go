...
//	./onepassword-bootstrap --vault <vault-name>
//
// Items created (idempotent: skips if title already exists):
//   - jason@host-b   (Login: Win PC WSL Ubuntu Login GB password)
//   - jason@host-d   (Login: Win PC WSL Ubuntu Login GB password)
...
func main() {
	vaultName := flag.String("vault", "<vault-name>", "1Password vault name (must be accessible by the service account)")
	...
	items := []itemSpec{
		{
			Title:    "Win PC WSL Ubuntu Login GB / jason@host-b",
			...
			Fields: []onepassword.ItemField{
				...
				{ID: "host", Title: "host", Value: "host-b", FieldType: onepassword.ItemFieldTypeText},
		...
		{
			Title:    "Win PC WSL Ubuntu Login GB / jason@host-d",
			Category: onepassword.ItemCategoryLogin,
			Fields: []onepassword.ItemField{
				{ID: "username", Title: "username", Value: "jason", FieldType: onepassword.ItemFieldTypeText},
				{ID: "password", Title: "password", Value: *password, FieldType: onepassword.ItemFieldTypeConcealed},
				{ID: "host", Title: "host", Value: "host-d", FieldType: onepassword.ItemFieldTypeText},
				{ID: "notes", Title: "notes", Value: "Auto-created by helixon-platform v14508.5 bootstrap. Universal Windows password; same value as 'Win PC WSL Ubuntu Login GB' Login item.", FieldType: onepassword.ItemFieldTypeText},
			},
		},
	}
	if *hfToken != "" {
		items = append(items, itemSpec{
			...
			Fields: []onepassword.ItemField{
				...
				{ID: "notes", Title: "notes", Value: "Auto-created by helixon-platform v14508.5 bootstrap. Injected via `op read op://<vault-name>/HF_TOKEN/credential` (CLI) or one.password-sdk-go Secrets().Resolve (programmatic).", FieldType: onepassword.ItemFieldTypeText},
			},
		})
	}
