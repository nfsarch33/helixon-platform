...
Renders 5 resources cleanly:
  ...
  Role
  RoleBinding
  Deployment  helixon-runner-gitlab-runner (pinned to node-a)
...
## Why this works for node-a

- Chart uses Kubernetes executor (no persistent storage required)
- Pod is pinned to node-a via `nodeSelector` + `nodeAffinity`
...
2. Register with GitLab (name: `helixon-node-a`, tags: `helixon,helixon-fleet`)
3. Show up in GitLab UI as `helixon-node-a`
