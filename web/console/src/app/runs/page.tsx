    ...
    <Panel
      ...
      actions={
        <label className="text-sm">
          <span className="mr-2 text-slate-600 dark:text-slate-400">Status</span>
      ...
      {error ? <ErrorState error={error} /> : !data ? <Loading /> : data.runs.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="text-slate-600 dark:text-slate-400"><tr><th scope="col" className="py-1 pr-3">Run</th><th scope="col" className="py-1 pr-3">Status</th><th scope="col" className="py-1 pr-3">Message</th><th scope="col" className="py-1 pr-3">Iter.</th><th scope="col" className="py-1 pr-3">Tokens</th><th scope="col" className="py-1 pr-3">Attempts</th><th scope="col" className="py-1">Updated</th></tr></thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {data.runs.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="max-w-[40ch] py-1 pr-3 wrap-anywhere">{truncate(r.user_message, 70)}</td>
                  <td className="py-1 pr-3">{r.iterations}</td>
                  <td className="whitespace-nowrap py-1 pr-3 tabular-nums">{fmtInt(r.tokens_in)} / {fmtInt(r.tokens_out)}</td>
                  <td className="py-1 pr-3">{r.attempts}</td>
                  <td className="whitespace-nowrap py-1 text-slate-600 dark:text-slate-400">{fmtTime(r.updated_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
