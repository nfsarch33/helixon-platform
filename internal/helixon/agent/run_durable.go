...
func (a *Agent) finish(ctx context.Context, run *RunRecord, result *RunResult, loopErr error, renew *renewState) (*RunResult, error) {
	if errors.Is(ctx.Err(), context.Canceled) {
		if renew != nil && renew.lost.Load() {
			// The renewer canceled this attempt because the lease is gone.
			// Tag it so the caller (the ticket poller) can abandon the ticket
			// instead of escalating: the run is resumable and the recovery
			// sweep, not a human, owns finishing it.
			if loopErr == nil {
				loopErr = ctx.Err()
			}
