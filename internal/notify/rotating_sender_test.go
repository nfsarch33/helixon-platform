...
func TestRotatingSender_RotatesAcrossSends(t *testing.T) {
	...
	if err := rs.Send(ctx, Email{To: []string{"ops@example.com"}, IdempotencyKey: "r1"}); err != nil {
	...
	if err := rs.Send(ctx, Email{To: []string{"ops@example.com"}, IdempotencyKey: "r2"}); err != nil {
	...
	if err := rs.Send(ctx, Email{To: []string{"ops@example.com"}, IdempotencyKey: "r3"}); err != nil {
...
func TestRotatingSender_FallsBackWhenDBNil(t *testing.T) {
	...
	if err := rs.Send(context.Background(), Email{
		To: []string{"ops@example.com"}, IdempotencyKey: "nil-db-1",
	}); err != nil {
		t.Fatalf("send: %v", err)
	}
	if c1Calls.Load() != 1 {
...
func TestRotatingSender_EndToEnd_WithHTTPServer(t *testing.T) {
	...
	if err := rs.Send(context.Background(), Email{
		To: []string{"ops@example.com"}, IdempotencyKey: "e2e-1", Subject: "hi",
	}); err != nil {
		t.Errorf("Send: %v", err)
	}
}
