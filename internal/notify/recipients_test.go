...
//
// Notification target per v16101 + CARRY-044 is ops@example.com only.
...
func TestCanonicalTargets_OnlyJaslian(t *testing.T) {
	...
	if got[0] != "ops@example.com" {
		t.Errorf("expected ops@example.com, got %s", got[0])
	}
}

func TestValidateRecipients_AcceptsCanonical(t *testing.T) {
	if err := ValidateRecipients([]string{"ops@example.com"}); err != nil {
...
func TestValidateRecipients_RejectsNonCanonical(t *testing.T) {
	tests := [][]string{
		{"ops@example.com"},
		{"someone@example.com"},
		{"ops@example.com", "cc@example.com"},
		{""},
		{"info@oztac.com.au"},
	}
...
func TestValidateRecipients_RejectsMixed(t *testing.T) {
	...
	err := ValidateRecipients([]string{"ops@example.com", "extra@example.com"})
	if !errors.Is(err, ErrNonCanonicalRecipient) {
