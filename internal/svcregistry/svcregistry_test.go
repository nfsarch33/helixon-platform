...
func TestServiceValidate_AcceptsCanonicalEntry(t *testing.T) {
	s := makeService("helixon-platform", "node-c", 8080, "http")
...
func TestRegistryRegister_OK(t *testing.T) {
	...
	reg := New(path)
	if err := reg.Register(makeService("helixon-platform", "node-c", 8080, "http")); err != nil {
...
func TestRegistrySaveLoad_RoundTrip(t *testing.T) {
	...
	path := filepath.Join(dir, "svc-registry.json")
	reg := New(path)
	for i, h := range []string{"node-b", "node-c", "oracle-jump"} {
	...
	if !bytes.Contains(data, []byte("svc-node-c")) {
		t.Fatalf("snapshot missing node-c: %s", data)
	}
	reg2 := New(path)
...
func TestHTTPServer_PostAndGet(t *testing.T) {
	...
	svc := makeService("helixon-platform", "node-c", 8080, "http")
	...
	resp3, err := http.Get(srv.URL + "/api/v1/services/node-c/helixon-platform")
	if err != nil {
		t.Fatal(err)
	}
