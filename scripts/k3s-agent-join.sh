...
# Usage:
#   K3S_URL=https://100.64.0.1:6443 K3S_TOKEN=<token> bash k3s-agent-join.sh [node-alias]
#
# Where:
#   K3S_URL is the k3s server reachable from the agent (node-a's Tailscale IP)
#   K3S_TOKEN is the cluster join token (from `cat /var/lib/rancher/k3s/server/node-token` on node-a)
#   node-alias is the human-friendly name (default: hostname)

set -euo pipefail

NODE_ALIAS="${1:-$(hostname)}"
K3S_URL="${K3S_URL:?must be set, e.g. https://100.64.0.1:6443}"
...
sleep 30
echo ">> done. Verify on node-a with: kubectl get nodes -o wide | grep $NODE_ALIAS"
