...
set +e
cd /home/user/Code/helixon-platform
...
# Templates like `op://<vault-name>/<uuid>/<field>` are SAFE — they're runtime lookups.
...
plain=$(grep -rE "(password|api_key|apiKey|secret|token|webhook_url|private_key|credential|notesPlain)[\"']?\s*[:=]\s*[\"']?[A-Za-z0-9+/=_-]{20,}" \
  ...
  /home/user/Code/helixon-platform/evidence/ 2>/dev/null \
  | grep -v "sha256:" \
...
missing=$(python3 -c "
import yaml
with open('/home/user/Code/cursor-global-kb/inventory/services/registry.yaml') as f:
    d = yaml.safe_load(f)
svcs = d.get('services', [])
...
token=$(grep -E "auth_token|require_auth" /home/user/Code/cursor-global-kb/configs/llm-cluster-router.yml 2>/dev/null | wc -l)
if [ "$token" -gt 0 ]; then
...
embed=$(grep -E "ENGRAM_EMBED_URL|embed_url" /home/user/.config/systemd/user/engram.service 2>/dev/null | wc -l)
if [ "$embed" -gt 0 ]; then
...
if [ -f /home/user/Code/cursor-global-kb/adrs/ADR-0098-host-a-node-a-production-readiness.md ]; then
...
if [ -f /home/user/Code/cursor-global-kb/sop/oracle-cloud-web-ui-automation.md ]; then
...
if [ -f /home/user/Code/cursor-global-kb/sop/1password-wsl-password-migration-strategy.md ]; then
...
dup_count=$(grep -c "tailscale_hostname: desktop-fh3nbqn-" /home/user/Code/cursor-global-kb/fleet/nodes.yaml 2>/dev/null | head -1)
...
hook_chk11=$(grep -c "Check 11" /home/user/Code/cursor-global-kb/sop/before-shell-execution-hook-spec.md 2>/dev/null | head -1)
...
uncommitted=$(cd /home/user/Code/helixon-platform && git status --short 2>/dev/null | wc -l | head -1)
uncommitted_kb=$(cd /home/user/Code/cursor-global-kb && git status --short 2>/dev/null | wc -l | head -1)
...
# Check 10: host-d LAN discovery evidence present (CF-v14577-02)
if [ -f /home/user/Code/cursor-global-kb/evidence/v14595-mesh/mesh-direct.json ]; then
...
if [ -f /home/user/Code/cursor-global-kb/evidence/v14594-host-d-lan/README.md ]; then
  check v14594-evidence-present PASS "v14594-host-d-lan/README.md committed"
else
  check v14594-evidence-present FAIL "v14594-host-d-lan/README.md missing"
fi

# Check 11: nodes.yaml at v10 (v14594 closing)
nodes_ver=$(grep -oE 'v[0-9]+ -- 2026-07-10' /home/user/Code/cursor-global-kb/fleet/nodes.yaml | tail -1 | grep -oE 'v[0-9]+' | grep -oE '[0-9]+')
...
# Check 12: node-d SSH via fleet_lan pubkey (CF-v14593-01 prerequisite)
if ssh -o ConnectTimeout=5 -o UserKnownHostsFile=/dev/null -o BatchMode=yes node-d 'whoami' 2>&1 | grep -q jason; then
  check node-d-fleet-lan-ssh PASS "node-d SSH via fleet_lan works"
else
  check node-d-fleet-lan-ssh FAIL "node-d SSH via fleet_lan broken"
fi

# Check 13: node-b/node-c/node-d SSH mesh from node-a (all OK)
mesh_ok=0
for tgt in node-b fleet-host-c-node-c node-d; do
...
if [ "$mesh_ok" -eq 3 ]; then
  check wsl-mesh-3of3 PASS "node-a -> node-b/node-c/node-d SSH mesh 3/3"
else
  check wsl-mesh-3of3 FAIL "node-a -> node-b/node-c/node-d SSH mesh only $mesh_ok/3"
fi

# Check 14: llm-cluster-router config has auth_token (CF-v14593-01)
auth_lines=$(grep -E "auth_token|require_auth" /home/user/Code/cursor-global-kb/configs/llm-cluster-router.yml 2>/dev/null | wc -l | head -1)
...
if grep -q "LLM_ROUTER_TOKEN" /home/user/Code/cursor-global-kb/configs/llm-cluster-router.yml 2>/dev/null; then
...
if grep -q "hfri3ziy6cjfec4xha7wkfkkri" /home/user/Code/helixon-platform/cmd/secrets-bootstrap/main.go 2>/dev/null; then
...
if ps aux | grep -E "op (daemon|item)" | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null; then
  if OP_SERVICE_ACCOUNT_TOKEN=$(cat ~/.config/op/service-account-token) \
       op item get hfri3ziy6cjfec4xha7wkfkkri --vault <vault-name> --format=json >/dev/null 2>&1; then
...
# Check 18: node-a -> host-a LAN SSH works (via fleet_lan)
if ssh -o ConnectTimeout=5 -o UserKnownHostsFile=/dev/null -o BatchMode=yes fleet-host-a-host-a 'whoami' 2>&1 | grep -q jaslian; then
  check host-a-lan-ssh PASS "node-a -> host-a SSH via fleet_lan works"
else
  check host-a-lan-ssh FAIL "node-a -> host-a SSH via fleet_lan broken"
fi

# Check 19: node-a -> host-c LAN SSH works (via fleet_lan, keynear user)
if ssh -o ConnectTimeout=5 -o UserKnownHostsFile=/dev/null -o BatchMode=yes -l keynear 100.64.0.5 'whoami' 2>&1 | grep -q keynear; then
  check host-c-lan-ssh PASS "node-a -> host-c SSH via fleet_lan works (keynear user)"
else
  check host-c-lan-ssh FAIL "node-a -> host-c SSH via fleet_lan broken"
fi
...
if cd /home/user/Code/helixon-platform && go test ./internal/svcregistry/ -run TestService_Accepts26CharUUIDName -count=1 2>&1 | grep -qE "(^| )PASS: TestService_Accepts26CharUUIDName|ok[[:space:]]+github"; then
...
if /home/user/local/bin/svcregistry-bridge -api http://127.0.0.1:7777 2>&1 | grep -q "failed=0"; then
