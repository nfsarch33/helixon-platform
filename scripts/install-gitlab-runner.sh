...
#
# Installs gitlab-runner on the node-a k3s cluster using the chart values
...
if [[ -z "$REG_TOKEN" ]]; then
    if command -v op >/dev/null 2>&1; then
        # 1Password item: GitLab CE API PAT (node-a) - reuse as registration token
        REG_TOKEN=$(op read "op://<vault-name>/GitLab CE API PAT (node-a)/credential" 2>/dev/null || true)
    fi
    if [[ -z "$REG_TOKEN" ]]; then
