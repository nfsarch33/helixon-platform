...
#
# Installs Argo CD on node-a's k3s cluster with:
...
#   4. Pinned nodeSelector to node-a (central node) via values.yaml
#
...
CHART_TGZ="/home/user/.cache/helm/repository/argo-cd-10.1.2.tgz"
...
if ! sudo k3s kubectl -n argocd get secret argocd-redis >/dev/null 2>&1; then
    ...
    mkdir -p /home/user/.cache/helixon
    echo "$REDIS_PASS" > /home/user/.cache/helixon/argocd-redis-pass
